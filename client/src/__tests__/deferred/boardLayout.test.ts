...
describe('Image board layout — base game', () => {
  ...
  it('marker anchors derive from per-hotspot agentX/agentY', () => {
    for (const anchor of MARKER_ANCHORS) {
      ...
      const expected = layoutAgentAnchorPercent(hotspot!)
      expect(anchor.x).toBeCloseTo(expected.x, 1)
      expect(anchor.y).toBeCloseTo(expected.y, 1)
    }
  })
